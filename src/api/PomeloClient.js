...
(function(root, ByteArray) {
  ...
  Emitter.prototype.off = Emitter.prototype.removeListener = Emitter.prototype.removeAllListeners = function(
    event,
    fn
  ) {
    this._callbacks = this._callbacks || {};
        ...
        switch (proto.option) {
          ...
          case "optional":
            offset = writeBytes(
              buffer,
              offset,
              encodeTag(proto.type, proto.tag)
            );
    ...
    switch (type) {
      ...
      default:
        if (!!protos.__messages[type]) {
          ...
          var tmpBuffer = new ArrayBuffer(
            codec.byteLength(JSON.stringify(value))
          );
          var length = 0;
  ...
  function encodeArray(array, proto, offset, buffer, protos) {
    ...
    if (util.isSimpleType(proto.type)) {
      ...
      offset = writeBytes(
        buffer,
        offset,
        Protobuf.codec.encodeUInt32(array.length)
      );
  ...
  pomelo.heartbeat = function(data) {
    ...
    this.heartbeatId = setTimeout(function() {
      ...
      self.heartbeatTimeoutId = setTimeout(
        self.heartbeatTimeoutCb.bind(self),
